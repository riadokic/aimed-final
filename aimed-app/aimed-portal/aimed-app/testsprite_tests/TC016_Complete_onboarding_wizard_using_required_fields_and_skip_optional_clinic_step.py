...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Prijava' (login) link to navigate to the login page (element index 99).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div[2]/div[2]/nav/div/div[2]/a[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Reload' button on the error page to attempt to load the /login page again (element index 74).
        frame = context.pages[-1]
        # Click element
        ...
        # -> Retry login by refilling email and password and clicking the submit button. If login fails again, stop and report the website issue.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=html/body/div[2]/div/div[3]/form/div[1]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('example@gmail.com')
