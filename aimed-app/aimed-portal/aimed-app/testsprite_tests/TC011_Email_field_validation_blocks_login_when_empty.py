...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Prijava' button (index 102) again to open the login page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div[2]/div[2]/nav/div/div[2]/a[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type 'password123' into the password field (index 1014) while leaving the email field empty, then click the 'Prijavite se' submit button (index 1015) to verify submission is prevented.
        frame = context.pages[-1]
        # Input text
        ...
        email_locator = frame.locator("text=Email")
        assert await email_locator.is_visible(), "Expected 'Email' text to be visible on the page"
