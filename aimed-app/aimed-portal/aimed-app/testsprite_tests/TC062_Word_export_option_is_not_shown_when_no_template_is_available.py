...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Prijava' link again to navigate to the login page (element index 102) and wait for the page to load.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=html/body/div[2]/div[2]/nav/div/div[2]/a[1]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type login email into the email field (index 1070) with example@gmail.com, then password (index 1074), then click the login button (index 1075).
        frame = context.pages[-1]
        # Input text
