...
async function submitOnce(options: SubmitOptions): Promise<string> {
  ...
  if (options.mode === "template" && options.template) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new AimedApiError(
        ...
        false // Don't auto-retry server errors
      );
    }
...
export async function submitRecording(options: SubmitOptions): Promise<string> {
