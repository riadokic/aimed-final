...
import { loadHistory, type HistoryEntry } from "@/lib/report-storage";
...
export default function HomePage() {
  ...
  const stats = useMemo(() => {
    ...
    const today = history.filter((e) => new Date(e.createdAt) >= startOfDay).length;
    const thisMonth = history.filter((e) => new Date(e.createdAt) >= startOfMonth).length;

    const totalMinSaved = history.reduce(
      (sum, e) => sum + calculateTimeSaved(e.sections),
      0
    );
