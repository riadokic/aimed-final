...
const plans = [
    {
        ...
        features: [
            "Do 120 diktata mjesečno",
            "Lokalni jezici (BiH, HR, SRB)",
    ...
    },
    {
        ...
        period: "/ mjesečno",
        description:
            "Za ljekare koji žele maksimalnu efikasnost bez limita.",
        features: [
            "Neograničen broj diktata",
            "Lokalni jezici (BiH, HR, SRB)",
            "Prioritena telefonska i email podrška",
            "Napredna medicinska terminologija",
        ],
...
export default function PricingLegacy() {
    return (
        <section
            id="pricing"
            className="min-h-screen flex items-center py-20 px-8 bg-white border-t border-zinc-100"
        >
            <div className="max-w-5xl mx-auto w-full">
              ...
              `}
                        >
                            <div className="mb-10">
                                <h3
                                    className={`text-[11px] font-bold mb-4 uppercase tracking-[0.3em] ${plan.highlighted ? "text-zinc-500" : "text-zinc-400"}`}
                                >
                                    {plan.name}
                                </h3>
                                <div className="flex items-baseline gap-1">
                                    ...
                                    <span
                                        className={`text-sm font-medium ${plan.highlighted ? "opacity-40" : "text-zinc-400"}`}
                                    >
                                        {plan.period}
                                    </span>
                                </div>
                                <p
                                    className={`mt-4 text-lg font-normal ${plan.highlighted ? "text-zinc-400" : "text-zinc-500"}`}
                                >
                                    {plan.description}
                                </p>
                            </div>
                            ...
                            <div className="space-y-4 mb-12 flex-1">
                                {plan.features.map((f, i) => (
                                    <div key={i} className="flex items-start gap-3 min-h-[28px]">
                                        <div
                                            className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${plan.highlighted ? "bg-white/10" : "bg-black/5"}`}
                                        >
                                            <Check
                                                className={`w-3 h-3 ${plan.highlighted ? "text-white" : "text-black"}`}
                                            />
                                        </div>
                                        <span
                                            className={`text-[15px] font-medium leading-[1.6] ${plan.highlighted ? "text-zinc-300" : "text-zinc-600"}`}
                                        >
                                            {f}
                                        </span>
                                    </div>
